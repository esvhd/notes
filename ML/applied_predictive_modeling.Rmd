...
distorted by different scale graphically. Components are stored in `precomp()`
result as `x`.
...
Alternatively, use `model.matrix()`
...
a value between -1 and +1, and can be found in package `mccr`. 
...
mccSummary <- function(data, lev = NULL, model = NULL) {
  ...
  pred.probs <- as.matrix(data[, lvls])
...
ROC plots False Positive Rate (FPR) on x-ais and Sensitivity (TPR) on y-axis.
...
`caret::downSample()` and `caret::upSample()`
...
model performance can become **biased**.
...
Odds ratio represent the increase in the odds of the even when going from the
