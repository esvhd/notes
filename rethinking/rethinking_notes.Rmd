...
Maximizing the joint probability of data is the correct way to identify the 
right model. E.g. Multiplying the probabilities of each correct prediction (i.e.
computed by using the prob density function of the model of data).

Taking the average is **not** the right way to do it.

## Information Theory

The measurement of information should satisfy:

* should be continuous
* should increase as the number of possible evens increases
* should be additive

### Entropy

The probabilities $p_i$ of data is computed as as follows: first, have a 
probabilistic model of the underlying event, this is a probability distribution,
using its probability density function, compute the probability $p_i$ of data
using this probabilistic model. 

$$ H(p) = -\mathbb{E}(\log(p_i)) = -\sum_{i=1}^{n}p_i \log(p_i) $$

### Cross Entropy

Minimizing cross-entropy is essentially minimizing the distance between the 
model and the underlying true distribution (which is unknown).
...
$$ D_{KL} = \sum_i p_i(\log(p_i) - \log(q_i)) = \sum_i p_i \log\Big(\frac{p_i}{q_i}\Big) $$
...
Deviance has the same flaw as $R^2$, it always improves as the model gets more
complex. Deviance in-sample is **not** a measure of out of sample predictive 
accuracy.
...
Deviance provides a measure for in-sample / training data model evaluation. 
We need a measure to evaluate models for for out-of-sample / test data. 
This is where AIC, DIC and WAIC come in.

The validity of these metrics depends upon the predictive task at hand.

My opinion: these are metrics designed to estimate OOS deviance, not sure in 
practice these work well. Maybe when you have lots of data, cross validation
is just a more practical method to tune the models, and compare models based
on test errors.

### AIC, page 189

AIC is reliable only when:
...
3. In the case of flat priors, DIC reduces to AIC.

Because DIC also assumes multivariate Gaussian posterior distributon, if any 
parameter in the posterior is substantially skewed, **and** also has a 
substantial effect on the prediction, then DIC like AIC can go horribly wrong.

$$ DIC = \bar{D} + (\bar{D} - \hat{D}) = \bar{D} + p_D $$
Where:

$D$ is the posterier distribution of deviance

$\bar{D}$ is the average of $D$

$\hat{D}$ is the deviance calculated at the posterier mean

$p_D$, known as the penalty term, is just the expected distance between the 
deviance in-sample and the deviance out-of-sample.
...
WAIC requires splitting data into **independent** observations. In time series,
where an observation may depend on the previous observation, this is **not**
satisfied, in this case it is **not** clear what the calculated WAIC means.
...
* `rethinking::compare()` compares models using WAIC. p199
    ...
    + the weights are **Akaike weights** for each model. Analogous to 
    posterior probabilities of models, conditional on expected future data.
...
**Bayes Factor** is a ratio of average likelihoods (the denominator in Bayes
theorem). There are technical issues related to using bayes factor as a measure
of OOS model evaluation.
...
```
milk.ensemble <- ensemble(model1, model2, model3, data=d.predict)
```
...
$exp(x)$ which is always > 0. Example:

```
a.start <- mean(d$kcal.per.g)
sigma.start <- log(sd(d$kcal.per.g))  # input sigmal is in log scalue
m6.11 <- map(
  alist(
...
* Use `rethinking::map2stan`
    ...
    + `warmup=` iterations of warm-up phase. These are thrown away.
...
**Conventional link for Possion models is the log link.**
...
Model likelihood with `rethinking::dcategorical()`, such a model built on 
multinomial distribution may be also called categorical regression model.

## Geometric Regression, p328

Conventional link is also `logit`

`stats::dgeom()` function. PDD:
...
Multilevel models deserves to be the **default** for regression problems. 
They can reveal issues not captured by simple models. It's better to try 
multilevel first and then realise it is not necessary than to ignore potential 
issues.

**Cost** of the multilevel approach:

1. They instroduce additinal assumptions in the modelling process. 
Have to define the distributions from which the characteristics of the clusters
arise. Using maximum entropy priors mitigates this issue somewhat.

2. New estimation challenges. Leads us to MCMC estimation.

3. Can be hard to understand, for model comparison, metrics such as `DIC` and
`WAIC` becomes more subtle. Logic is unchanged, but need to decide which 
...
An example model below. Note that $\alpha_{tank}$ itself is modelled as a 
parameter.

Here, $\alpha$ and $\sigma$ are **hyperparameters**, 
and their priors are **hyperpriors**. 

The model below has **less** effective
parameters than an ordinary model, because the two extra parameters in the 
multilevel model allowed it to learn a **more aggressive** regularizing
prior, to adaptively regularize, resulting in **less** flexible posterior
and therefore **fewer** effective parameters.

$$
\begin{aligned}
s_i &\sim \mathrm{Binomial}(n_i, p_i)\\
logit(p_i) &= \alpha_{tank}[i] \\
\alpha_{tank} &\sim \mathrm{Normal}(\alpha, \sigma) \\
\alpha &\sim \mathrm{Normal}(0, 1) \\
\sigma &\sim \mathrm{HalfCauchy}(0, 1)
\end{aligned}
$$

Cauchy priors work well in routine multilevel model, except in two cases:

1. There isn't much info in the data with which we use to estimate the 
variance. E.g. only 5 clusters, then this is like estimating variance
with 5 data points.

2. In non-linear models with logit and log links, **floor** and **ceiling**
effects sometimes render extreme values of the variance equally plausible as
more realistic values.

Cauchy has very **thick** and **long** tails, comparing to Exponential 
distribution which has much **thinner** tails.

In such cases, we can replace Cauchy with Exponential priors. Typically, it is
useful to try different priors to ensure that the inference either is 
**insensitive** to them or rather to measure how inference is **altered**.
...
**Partial Pooling** - reduce effects of both above. This is what happens when
estimating varying intercepts.
...
better (avoids divergent iterations warning from `Stan`) but not always the 
case. See practice 13M3. 
...
and correlation. `dmvnormNC()` does this for us. Note with `dmvnormNC()`, only
`sigma` and `Rho` (standard deviation and correlation) need to be provided.
The mean should always be **in the model, not the prior**, for non-centered 
parameterization.
...
m <- map2stan(
  alist(
    ...
    neocortex ~ dnorm(nu, sigma_N),  # missing value drawn based on this line
    a ~ dnorm(0, 100),
    c(bN, bM) ~ dnorm(0, 10),
    nu ~ dnorm(0.5, 1),
    sigma_N ~ dcauchy(0, 1),
  ...
  data=data_list, iter=4e4, chains=2
)
```
...
m <- map2stan(
  alist(
    ...
    nu <- a_N + gM * logmass,        # this is the linear model
    a ~ dnorm(0, 100),
